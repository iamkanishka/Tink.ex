defmodule Tink.RiskInsights do
  ...
  ## Flow
      ...
      client = Tink.client(scope: "risk-insights:readonly")

      # Step 3: Retrieve risk insights report
      {:ok, report} = Tink.RiskInsights.get_report(client, risk_insights_id)

  ## Use Cases

  ### Comprehensive Risk Assessment

      @spec perform_risk_assessment(Client.t(), String.t()) :: {:ok, map()} | {:error, Error.t()}
  ...
  ### Early Warning Detection
      ...
      @spec detect_financial_stress(Client.t(), String.t()) :: {:ok, map()} | {:error, Error.t()}
  ...
  ### Portfolio Risk Management
      ...
      @spec assess_portfolio_risk(Client.t(), [String.t()]) :: {:ok, map()} | {:error, Error.t()}
  ...
  ### Credit Limit Adjustment
      ...
      @spec recommend_credit_limit_adjustment(Client.t(), String.t(), integer()) :: {:ok, map()} | {:error, Error.t()}
  ...
  ## Parameters
    ...
    * `client` - Tink client with `risk-insights:readonly` scope
  ...
  ## Examples
      ...
      {:ok, report} = Tink.RiskInsights.get_report(client, "risk_ins_abc123")
      #=> {:ok, %{
      #     "id" => "risk_ins_abc123",
      #     "userId" => "user_123",
      #     "status" => "COMPLETED",
  ...
  ## Use Cases
      ...
      {:ok, report} = Tink.RiskInsights.get_report(client, report_id)
  ...
  def get_report(%Client{} = client, report_id) when is_binary(report_id) do
    ...
    if client.cache && Cache.enabled?() do
      cache_key = Cache.build_key(["risk-insights", report_id])
