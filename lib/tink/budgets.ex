defmodule Tink.Budgets do
  ...
  def get_budget(%Client{} = client, budget_id) when is_binary(budget_id) do
    ...
    if client.cache && Cache.enabled?() do
      cache_key = Cache.build_key([client.user_id || "public", "budgets", budget_id])

      Cache.fetch(cache_key, fn -> Client.get(client, url, cache: false) end, resource_type: :default)
    else
  ...
  def list_budgets(%Client{} = client, opts \\ []) do
    ...
    if client.cache && Cache.enabled?() do
      cache_key = Cache.build_key([client.user_id || "public", "budgets", URI.encode_query(opts)])
  ...
  def update_budget(%Client{} = client, budget_id, updates)
    ...
    url = "/finance-management/v1/business-budgets/#{budget_id}"
    result = Client.patch(client, url, updates)

    if match?({:ok, _}, result) do
      Cache.delete(Cache.build_key([client.user_id || "public", "budgets", budget_id]))
    end
