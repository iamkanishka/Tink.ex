defmodule Tink.Config do
  ...
  # Tink uses same base URL for both environments; credentials determine sandbox vs prod
  @sandbox_url "https://api.tink.com"
  ...
  def get_all do
    app_config = Application.get_all_env(:tink)
  ...
  defp missing_required?(config) do
    # access_token OR (client_id AND client_secret) required
  ...
  defp invalid_environment?(config) do
    Keyword.get(config, :environment) not in [:production, :sandbox, nil]
  end
  ...
  defp resolve_base_url(%{base_url: url} = config) when is_binary(url), do: config

  defp resolve_base_url(%{environment: :sandbox} = config),
    do: Map.put(config, :base_url, @sandbox_url)

  defp resolve_base_url(%{environment: :production} = config),
    do: Map.put(config, :base_url, @production_url)

  defp resolve_base_url(config), do: Map.put(config, :base_url, @production_url)
end
