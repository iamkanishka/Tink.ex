defmodule Tink.ExpenseCheck do
  ...
  ## Flow
      ...
      client = Tink.client(scope: "expense-checks:readonly")

      # Step 3: Retrieve expense report
      {:ok, report} = Tink.ExpenseCheck.get_report(client, expense_check_id)

  ## Use Cases

  ### Loan Affordability Assessment

      @spec assess_loan_affordability(Client.t(), String.t(), number()) :: {:ok, map()} | {:error, Error.t()}
  ...
  ### Subscription Service Verification
      ...
      @spec verify_subscription_affordability(Client.t(), String.t(), number()) :: {:ok, map()} | {:error, Error.t()}
  ...
  ### Expense Category Analysis
      ...
      @spec analyze_spending_by_category(Client.t(), String.t()) :: {:ok, map()} | {:error, Error.t()}
  ...
  ## Parameters
    ...
    * `client` - Tink client with `expense-checks:readonly` scope
  ...
  ## Examples
      ...
      {:ok, report} = Tink.ExpenseCheck.get_report(client, "expense_abc123")
      #=> {:ok, %{
      #     "id" => "expense_abc123",
      #     "userId" => "user_123",
      #     "period" => %{
  ...
  def get_report(%Client{} = client, report_id) when is_binary(report_id) do
    ...
    if client.cache && Cache.enabled?() do
      cache_key = Cache.build_key(["expense-check", report_id])
