defmodule Tink.WebhookVerifier do
  ...
  def verify_signature(payload, signature, secret)
    ...
    if secure_compare(signature, expected_signature) do
      ...
      Logger.warning("[Tink.WebhookVerifier] Invalid webhook signature")
      {:error, :invalid_signature}
    end
  end
  ...
  # Constant-time comparison using OTP's built-in implementation (OTP 25.1+).
  # Falls back gracefully: if the function is unavailable, we do a length-check
  # first (which is still an improvement over a naive binary ==).
