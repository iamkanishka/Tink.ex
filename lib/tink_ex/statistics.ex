defmodule TinkEx.Statistics do
  @moduledoc """
  ...
  ### Monthly Spending Report
      ...
      def generate_monthly_report(user_client, year, month) do
        ...
        {:ok, stats} = TinkEx.Statistics.get_statistics(user_client,
  ...
  ## Examples
      ...
      #           },
      #           %{
      #             "categoryId" => "expenses:transport",
      #             "categoryName" => "Transport",
      #             "amount" => %{"value" => 280.00, "currencyCode" => "GBP"},
      #             "transactionCount" => 8
      #           }
      #         ]
      #       },
      ...
      {:ok, weekly} = TinkEx.Statistics.get_statistics(user_client,
        period_gte: "2024-01-01",
      ...
      # Filter by account
      {:ok, checking_stats} = TinkEx.Statistics.get_statistics(user_client,
        period_gte: "2024-01-01",
        period_lte: "2024-12-31",
        resolution: "MONTHLY",
      ...
      # Filter by category
      {:ok, food_stats} = TinkEx.Statistics.get_statistics(user_client,
        period_gte: "2024-01-01",
        period_lte: "2024-12-31",
        resolution: "MONTHLY",
  ...
  ## Examples
      ...
      {:ok, grocery_stats} = TinkEx.Statistics.get_category_statistics(
        user_client,
  ...
  ## Examples
      ...
      {:ok, account_stats} = TinkEx.Statistics.get_account_statistics(
        user_client,
        "account_123",
        period_gte: "2024-01-01",
        period_lte: "2024-12-31",
...
defmodule TinkEx.Statistics do
  @moduledoc """
  Statistics API with intelligent caching (1 hour TTL).

  Statistics are aggregated data that's expensive to compute, making them
  ideal for caching.
  """
  ...
  @doc """
  Gets financial statistics for a time period with caching.

  Statistics are cached for 1 hour since they're aggregated historical data.
  ...
  def get_statistics(%Client{} = client, opts) do
    ...
    # :statistics resource type = 1 hour TTL
    Client.get(client, url)
  end

  @doc """
  Gets category-specific statistics with caching.
  """
  @spec get_category_statistics(Client.t(), String.t(), keyword()) ::
          {:ok, map()} | {:error, Error.t()}
  def get_category_statistics(%Client{} = client, category_id, opts)
  ...
  @doc """
  Gets account-specific statistics with caching.
  """
  @spec get_account_statistics(Client.t(), String.t(), keyword()) ::
          {:ok, map()} | {:error, Error.t()}
  def get_account_statistics(%Client{} = client, account_id, opts)
