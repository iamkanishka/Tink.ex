defmodule TinkEx.Error do
  @moduledoc """
  Error struct and handling for TinkEx.
  ...
  ## Examples
      ...
      case TinkEx.Accounts.list(client) do
        {:ok, accounts} ->
          # Success
          
        {:error, %TinkEx.Error{type: :rate_limit_error}} ->
          # Handle rate limit
          
        {:error, %TinkEx.Error{status: 401}} ->
          # Handle unauthorized
  ...
  ## Examples
      ...
      iex> TinkEx.Error.new(type: :network_error, message: "Connection failed")
      %TinkEx.Error{type: :network_error, message: "Connection failed"}
  """
  @spec new(keyword()) :: t()
  def new(attrs) do
  ...
  ## Examples
      ...
      iex> TinkEx.Error.from_response(400, %{"errorCode" => "INVALID_REQUEST"})
      %TinkEx.Error{type: :api_error, status: 400, error_code: "INVALID_REQUEST"}
  """
  ...
  ## Examples
      ...
      iex> TinkEx.Error.from_http_error(%{type: :timeout, reason: "Request timed out"})
      %TinkEx.Error{type: :timeout, message: "Request timed out"}
  """
  ...
  ## Examples
      ...
      iex> TinkEx.Error.retryable?(error)
      true
  ...
  ## Examples
      ...
      iex> error = TinkEx.Error.from_response(429, %{"errorMessage" => "Rate limit exceeded"})
      iex> TinkEx.Error.format(error)
  ...
  def format(%__MODULE__{} = error) do
    ...
    parts = [error.message | parts]

    parts =
      if error.error_code do
  ...
  defp error_type_from_status(status) when status in 400..499 do
    case status do
  ...
  defp error_type_from_status(status) when status in 500..599 do
    :api_error
  end
  ...
  defp retryable_type?(type) do
    type in [:network_error, :timeout]
  end
