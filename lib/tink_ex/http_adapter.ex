defmodule TinkEx.HTTPAdapter do
  @moduledoc """
  Production HTTP client implementation using Finch.

  This module implements the `TinkEx.HTTPBehaviour` interface and provides
  ...
  The adapter respects TinkEx configuration:
  ...
  - `[:tink_ex, :request, :start]` - Request initiated
  - `[:tink_ex, :request, :stop]` - Request completed
  - `[:tink_ex, :request, :exception]` - Request failed

  ## Examples

      # Direct usage
      {:ok, response} = TinkEx.HTTPAdapter.request(
        :get,
      ...
      # Used automatically by TinkEx.Client
      client = TinkEx.client()
      {:ok, accounts} = TinkEx.Accounts.list(client)
  """

  @behaviour TinkEx.HTTPBehaviour

  require Logger

  alias TinkEx.{Retry, Config}
  ...
  defp build_request(method, url, body, headers) do
    try do
      request =
        Finch.build(
          method,
  ...
  defp execute_request(request, timeout, receive_timeout) do
    case Finch.request(request, TinkEx.Finch,
  ...
  defp decode_response(%{body: _body} = response), do: {:ok, response}
