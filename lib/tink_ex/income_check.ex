defmodule TinkEx.IncomeCheck do
  @moduledoc """
  ...
  ## Flow
      ...
      client = TinkEx.client(scope: "income-checks:readonly")

      # Step 3: Retrieve income report
      {:ok, report} = TinkEx.IncomeCheck.get_report(client, income_check_id)

      # Step 4: (Optional) Generate PDF report
      {:ok, pdf_binary} = TinkEx.IncomeCheck.get_report_pdf(client, income_check_id)
  ...
  ## Parameters
    ...
    * `client` - TinkEx client with `income-checks:readonly` scope
  ...
  ## Examples
      ...
      {:ok, report} = TinkEx.IncomeCheck.get_report(client, "income_abc123")
      #=> {:ok, %{
      #     "id" => "income_abc123",
      #     "userId" => "user_123",
      #     "status" => "COMPLETED",
  ...
  ## Examples
      ...
      {:ok, pdf_binary} = TinkEx.IncomeCheck.get_report_pdf(client, "income_abc123")
  ...
  ## Use Cases
      ...
      def generate_income_verification_pdf(client, income_check_id, applicant_id) do
        case TinkEx.IncomeCheck.get_report_pdf(client, income_check_id) do
          {:ok, pdf_binary} ->
      ...
      def email_income_report(client, income_check_id, underwriter_email) do
        {:ok, pdf} = TinkEx.IncomeCheck.get_report_pdf(client, income_check_id)
