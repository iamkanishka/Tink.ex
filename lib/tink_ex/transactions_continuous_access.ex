defmodule TinkEx.TransactionsContinuousAccess do
  @moduledoc """
  ...
  ## Flow Overview
      ...
      {:ok, user} = TinkEx.TransactionsContinuousAccess.create_user(client, %{
      ...
      {:ok, grant} = TinkEx.TransactionsContinuousAccess.grant_user_access(grant_client, %{
      ...
      tink_link_url = TinkEx.TransactionsContinuousAccess.build_tink_link(grant["code"], %{
      ...
      {:ok, auth} = TinkEx.TransactionsContinuousAccess.create_authorization(auth_client, %{
        user_id: user["user_id"],
        scope: "accounts:read,balances:read,transactions:read,provider-consents:read"
      })

      # Step 6: Get user access token
      {:ok, token} = TinkEx.TransactionsContinuousAccess.get_user_access_token(client, auth["code"])

      # Step 7: Create authenticated client
      user_client = TinkEx.client(access_token: token["access_token"])

      # Step 8: Access data anytime
      {:ok, accounts} = TinkEx.TransactionsContinuousAccess.list_accounts(user_client)
      {:ok, transactions} = TinkEx.TransactionsContinuousAccess.list_transactions(user_client)
  ...
  ## Complete Example
      ...
      defmodule MyApp.ContinuousTransactions do
        def setup_user(external_user_id, email) do
          # Step 1: Create user
          create_client = TinkEx.client(scope: "user:create")
          {:ok, user} = TinkEx.TransactionsContinuousAccess.create_user(create_client, %{
          ...
          tink_link = TinkEx.TransactionsContinuousAccess.build_tink_link(grant["code"], %{
        ...
        def fetch_data(user_id) do
          ...
          with {:ok, accounts} <- TinkEx.TransactionsContinuousAccess.list_accounts(user_client),
               {:ok, transactions} <- TinkEx.TransactionsContinuousAccess.list_transactions(user_client) do
  ...
  def grant_user_access(%Client{} = client, params) when is_map(params) do
    ...
    actor_client_id = Application.get_env(:tink_ex, :actor_client_id, "df05e4b379934cd09963197cc855bfe9")
