defmodule TinkEx.TransactionsOneTimeAccess do
  @moduledoc """
  ...
  ## Flow Overview
      ...
      {:ok, accounts} = TinkEx.TransactionsOneTimeAccess.list_accounts(user_client)

      # Step 5: List transactions
      {:ok, transactions} = TinkEx.TransactionsOneTimeAccess.list_transactions(user_client)
  ...
  ### Quick Financial Snapshot
      ...
      def get_financial_snapshot(user_client) do
        with {:ok, accounts} <- TinkEx.TransactionsOneTimeAccess.list_accounts(user_client),
             {:ok, transactions} <- TinkEx.TransactionsOneTimeAccess.list_transactions(user_client,
  ...
  ## Parameters
    ...
    * `client` - TinkEx client with user access token (from authorization_code grant)
  ...
  ## Examples
      ...
      {:ok, recent} = TinkEx.TransactionsOneTimeAccess.list_transactions(user_client,
