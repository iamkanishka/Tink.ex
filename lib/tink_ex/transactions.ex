defmodule TinkEx.Transactions do
  @moduledoc """
  ...
  without creating a permanent user:
      ...
      # Step 4: Get accounts
      {:ok, accounts} = TinkEx.Transactions.list_accounts(user_client)

      # Step 5: Get transactions
      {:ok, transactions} = TinkEx.Transactions.list_transactions(user_client)
  ...
  Returns account information including balances, account numbers, and metadata.
  ...
  ## Returns
    ...
    * `{:ok, accounts}` - List of accounts with pagination info
    * `{:error, error}` - If the request fails

  ## Examples

      # After exchanging code for token
      {:ok, token} = TinkEx.Auth.exchange_code(client, code)
  ...
  ### Metadata
  - **dates**: Last refreshed timestamp
  ...
  ## Examples
      ...
      {:ok, recent} = TinkEx.Transactions.list_transactions(user_client,
        booked_date_gte: "2024-01-01",
        booked_date_lte: "2024-01-31"
      )

      # Filter by account
      {:ok, checking_txns} = TinkEx.Transactions.list_transactions(user_client,
        account_id_in: ["account_123"]
      )

      # Get only booked transactions
      {:ok, booked} = TinkEx.Transactions.list_transactions(user_client,
        status_in: ["BOOKED"]
      )

      # Combine filters
      {:ok, filtered} = TinkEx.Transactions.list_transactions(user_client,
      ...
      # Pagination
      {:ok, first_page} = TinkEx.Transactions.list_transactions(user_client,
        page_size: 50
      )

      if first_page["nextPageToken"] do
        {:ok, second_page} = TinkEx.Transactions.list_transactions(user_client,
  ...
  ## Use Cases
      ...
      {:ok, txns} = TinkEx.Transactions.list_transactions(user_client,
        booked_date_gte: "2024-01-01",
        status_in: ["BOOKED"]
      )
